...
import com.oyp.sort.bean.NetAreaCode;
import com.oyp.sort.strategy.EnglishSortStrategy;
import com.oyp.sort.strategy.ISortStrategy;
import com.oyp.sort.strategy.PinyinSortStrategy;
import com.oyp.sort.strategy.StrokeSortStrategy;
...
public class ChooseCountryOrRegionActivity extends Activity {
    ...
    private void initData() {
        ...
        Observable.just("")
                .map(new Func1<String, List<NetAreaCode>>() {
                    @Override
                    public List<NetAreaCode> call(String string) {
                        //模拟访问服务器获取到的数据
                        ...
                        List<NetAreaCode> areaCodeList = JSONUtil.toCollection(areaCodeListJson, List.class, NetAreaCode.class);
                        return areaCodeList;
                    }
                })
                .map(new Func1<List<NetAreaCode>, List<CountryOrRegion>>() {
                    @Override
                    public List<CountryOrRegion> call(List<NetAreaCode> areaCodeList) {
                        List<CountryOrRegion> countryOrRegionList = new ArrayList<>();
                        //批量转换为准备插入数据库的数据
                        for (int i = 0; i < areaCodeList.size(); i++) {
                            NetAreaCode areaCode = areaCodeList.get(i);
                            CountryOrRegion countryOrRegion = new CountryOrRegion();
                            countryOrRegion.setAreaCode(areaCode.getAreaCode());
                            countryOrRegion.setCountryCode(areaCode.getCountryCode());
                            countryOrRegionList.add(countryOrRegion);
                        }
